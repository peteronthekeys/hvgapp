...
- `site/SiteChrome.tsx` renders the optional `schema.site` page chrome (theme, sticky nav, footer, custom cursor, loading gate) additively into `PreviewPanel.tsx`; same purity law as `elements/` (no App/EditorPanel/ChatPanel imports — it's in the player bundle graph), and `LoadingGate` only renders when `PreviewPanel`'s `embedded` prop is true.
- `IntegrationsDrawer.tsx` is editor-only (mounted by `App.tsx`, never in the player/export graph) and owns `schema.integrations` — user-managed public site keys (form endpoint, reCAPTCHA v3 site key, Maps key). Integrations are user-owned, not AI-owned: `server/gemini.ts`'s `sanitizeSchema` strips any `integrations` the model hallucinates and `runChat` reattaches the caller's real values from `currentSchema` verbatim.
- `FormElementView.tsx`/`MapElementView.tsx` read `ctx.integrations` (threaded from `PreviewPanel.tsx`'s `schema.integrations` via `DomRendererCtx`, not imported directly) to find the form endpoint / Maps key; both are registered `interactive: true` and the map's iframe wrapper carries `data-lenis-prevent` — same laws as the carousel.
